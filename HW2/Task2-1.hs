...
data Equipment = Equipment { upgradeHealth :: Int, upgradeAttack :: Int, upgradeDefence :: Int }
  deriving Show
data Player = Player { phealth :: Int, pattack :: Int, pdefence :: Int }
  deriving Show
data Monster = Monster { mhealth :: Int, mattack :: Int, mdefence :: Int, mequipment :: Equipment }
  deriving Show
...
gloriousBattle p (m:ms)
  | (p & phealth) <= 0                              = p
  | (p & pattack) >= (m & mhealth) - (m & mdefence) = gloriousBattle (Player newHealth newAttack newDefence) ms
